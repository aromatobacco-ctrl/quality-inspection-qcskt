...
-- Pastikan role Guest diterima walaupun schema lama hanya mengizinkan
-- ADMIN dan INSPECTOR. Constraint role lama diganti secara aman.
...
begin
  ...
  loop
    execute format('alter table public.app_users drop constraint %I', v_constraint.conname);
  end loop;
end;
$$;
...
alter table public.app_users
  ...
  check (role in ('ADMIN', 'INSPECTOR', 'GUEST'));
...
begin
  ...
  -- Guest dapat membaca seluruh ringkasan, tetapi tidak dapat mengubah data
  -- walaupun endpoint dipanggil langsung di luar dashboard.
  if v_user.role = 'GUEST' and v_method <> 'GET' then
    return jsonb_build_object(
      '__error', 'Akun Tamu hanya memiliki akses lihat dan tidak dapat mengubah data.',
      '__status', 403
    );
  end if;
  ...
  if p_path = '/api/users' and v_method = 'POST' then
    ...
    if length(trim(coalesce(v_payload->>'displayName', ''))) < 2
       ...
       or coalesce(v_payload->>'role', '') not in ('ADMIN', 'INSPECTOR', 'GUEST')
  ...
  if p_path ~ '^/api/users/[0-9a-fA-F-]+$' and v_method = 'PATCH' then
    ...
    v_id := split_part(p_path, '/', 4)::uuid;
    select * into v_target from public.app_users where id = v_id;
    if not found then
      return jsonb_build_object('__error', 'Akun tidak ditemukan.', '__status', 404);
    end if;
    if v_id = v_user.id and (
      (v_payload ? 'isActive' and (v_payload->>'isActive')::boolean = false)
      or (v_payload ? 'role' and upper(trim(v_payload->>'role')) <> 'ADMIN')
    ) then
      return jsonb_build_object('__error', 'Admin tidak dapat menonaktifkan atau menurunkan role akunnya sendiri.', '__status', 400);
    end if;
    if v_payload ? 'role' and upper(trim(v_payload->>'role')) not in ('ADMIN', 'INSPECTOR', 'GUEST') then
      return jsonb_build_object('__error', 'Role akun tidak valid.', '__status', 400);
    end if;
    if v_payload ? 'displayName' and length(trim(v_payload->>'displayName')) < 2 then
      return jsonb_build_object('__error', 'Nama pengguna minimal 2 karakter.', '__status', 400);
    end if;
    if v_payload ? 'username' and trim(v_payload->>'username') !~ '^[A-Za-z0-9._-]{3,40}$' then
      return jsonb_build_object('__error', 'Username tidak valid.', '__status', 400);
    end if;
    if v_payload ? 'password' and (
      length(v_payload->>'password') < 8 or (v_payload->>'password') !~ '[A-Za-z]' or (v_payload->>'password') !~ '[0-9]'
    ) then
      return jsonb_build_object('__error', 'Password minimal 8 karakter dan harus mengandung huruf serta angka.', '__status', 400);
    end if;
    if v_target.role = 'ADMIN' and v_payload ? 'role' and upper(trim(v_payload->>'role')) <> 'ADMIN' then
      select count(*) into v_admin_count from public.app_users where role = 'ADMIN' and is_active = true;
      if v_admin_count <= 1 then
        return jsonb_build_object('__error', 'Minimal harus ada satu akun Admin aktif.', '__status', 400);
      end if;
    end if;
    begin
      update public.app_users
      set display_name = case when v_payload ? 'displayName' then trim(v_payload->>'displayName') else display_name end,
          username = case when v_payload ? 'username' then lower(trim(v_payload->>'username')) else username end,
          role = case when v_payload ? 'role' then upper(trim(v_payload->>'role')) else role end,
          is_active = case when v_payload ? 'isActive' then (v_payload->>'isActive')::boolean else is_active end,
          password_hash = case when v_payload ? 'password' and length(v_payload->>'password') > 0
            then extensions.crypt(v_payload->>'password', extensions.gen_salt('bf', 10)) else password_hash end,
          failed_login_count = case when v_payload ? 'password' then 0 else failed_login_count end,
          locked_until = case when v_payload ? 'password' then null else locked_until end
      where id = v_id;
    exception when unique_violation then
      return jsonb_build_object('__error', 'Username sudah dipakai.', '__status', 409);
    end;
    if (v_payload ? 'isActive' and (v_payload->>'isActive')::boolean = false) or v_payload ? 'password' then
      delete from public.app_sessions where user_id = v_id;
    end if;
    return jsonb_build_object('ok', true);
  end if;

  if p_path ~ '^/api/users/[0-9a-fA-F-]+$' and v_method = 'DELETE' then
    if v_user.role <> 'ADMIN' then
      return jsonb_build_object('__error', 'Hanya Admin yang dapat menghapus akun.', '__status', 403);
    end if;
    v_id := split_part(p_path, '/', 4)::uuid;
    select * into v_target from public.app_users where id = v_id;
    if not found then
    ...
    if v_id = v_user.id then
      return jsonb_build_object('__error', 'Admin tidak dapat menghapus akun yang sedang digunakan.', '__status', 400);
    end if;
    ...
    update public.inspections set inspector_id = null where inspector_id = v_id;
    update public.moisture_records set inspector_id = null where inspector_id = v_id;
    update public.pack_records set inspector_id = null where inspector_id = v_id;
    delete from public.app_sessions where user_id = v_id;
